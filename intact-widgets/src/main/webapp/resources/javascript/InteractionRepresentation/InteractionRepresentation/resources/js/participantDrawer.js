...
ParticipantDrawer = function(interactionInformation){
	...
	this._highlightingFeatures = new Object();
	this._curHighlightingFeature;
	this._curHighlightingFeatureColour;
    ...
    this.drawInteractor = function(participant, curFeatureTrackObject, y){
        ...
        if (interactorSymbol == "" && !(curFeatureTrackObject.annotations === undefined)) {
            ...
            if (!(curTracks === undefined)) {
				...
				interactorStart += this._interactionInformation._positionsOnProtein["top"];
            }
        }
        ...
        $(features).each(function(){
            for (var featureKey in this) {
				...
				if(!(feature.xref === undefined)){
					...
					}else if(feature.xref.primaryRef.refType == "source reference"){
					...
					if (featureId == "") {
						$(feature.xref.secondaryRef).each(function(){
							if (featureId == "" && this.refType == "identity" && !(this.db == "intact")) {
								...
								if (this.refType == "source reference" && this.db == "intact") {
                ...
                for (var categoryKey in self._interactionInformation._typeCategories) {
                    ...
                    if (insert) {
						...
						coordinates["colour"] = colour;
						if(featureId == ""){
							featureId = "intern" + feature.id;
						}
						...
						self._highlightingFeatures[featureId] = coordinates;
                    }
                }
            }
	...
	this.prepareEventHandling = function(element, highlightElements, connection){
		...
		$(highlightElements).each(function(){
			if ($.inArray(this, self._connectedElements[id].highlightElements) < 0) {
				...
				self._connectedElements[id].colours.push(this.attr("stroke"));
			}
		});
	}

    // draw a translucent rectangle around a protein to highlight the region on the protein
    // colors the rectangle in the given colour and displays the given text if available
	this.highlightRegion = function(interactorId, x, x2, colour, text){
		...
		if(!(this._interactors[interactorId] === undefined)){
			if (this._highlightingRegions[interactorId + x + x2] === undefined) {
				...
				$(this._interactors[interactorId]).each(function(){
					...
					var rect = self._shapeDrawer.getRectangle(x, y - 8, x2 - x, height + 8);
					rect.attr({
						"fill": colour,
						"stroke": colour,
						"stroke-width": "0.5",
					...
					if(!(text === undefined) || text != ""){
						var textDisplay = self._shapeDrawer.getText(x, y - 4, text);
						textDisplay.attr({
							"font-size": 9.5,
							"fill": colour
						});
						set.push(textDisplay);
					}
				});
				
				set.insertBefore(this._featureSet);
    ...
    // hide currently displayed highlighting rectangle
    ...
    // make the border of a feature black to highlight it
	this.highlightFeature = function(featureId){
		var feature = this._highlightingFeatures[featureId];
		this.unhighlightFeature();
		if(!(feature === undefined)){
			this._curHighlightingFeature = feature;
			this._curHighlightingFeature.element.attr("stroke", "black");
		} 
	}

    // color the border in the original color again
	this.unhighlightFeature = function(){
		if (!(this._curHighlightingFeature === undefined)) {
			this._curHighlightingFeature.element.attr("stroke", this._curHighlightingFeature.colour);
		}
		this.unhighlight(this._selectedInteraction);
	}
    ...
    this.unhighlight = function(interaction){
		...
		if (interaction.elements != null) {
			for (var i = 0; i < interaction.elements.length; i++) {
				interaction.elements[i].attr("stroke", interaction.colours[i]);
			}
		}
    };
    ...
    // calculate the width for the category part of the legend
    ...
    // calculate the width of the range type section of the legend
    ...
    // calculate the width ot the interactor type part of the legend
    ...
    // get text for the description of a range type in the legend
    ...
    // draw the range type part of the legend
    ...
    // draw the interaction type part of the legend
    ...
    // draw the category part of the legend
	...
	// draw legend items for the category part of the legend
