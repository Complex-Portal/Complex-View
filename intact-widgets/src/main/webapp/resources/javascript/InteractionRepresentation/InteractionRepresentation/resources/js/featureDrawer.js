FeatureDrawer = function(interactionInformation, shapeDrawer, featureSet){
   ...
   //  default drawing function for start < given position and undefined end
    ...
    // default drawing function for begin in a specific region and end < given position
    ...
    // default drawing function for undefined begin and end > given position
    ...
    // default drawing function for unrecognized rangetype
    ...
    // find the function to draw a symbol based on the given string
    ...
    // draw undetermined range status
    ...
    // text for range in the tooltip
	this.getRangeText = function(x,x2){
		return x + ".." + x2;
	};

    // text for greater than in the tooltip
	this.getGreaterThanText = function(x){
		return x + ">";
	};

    // text for less than in the tooltip
	this.getLessThanText = function(x){
		return "<" + x;
	};

    // creates an invisible element the same size of the given element to handle events
    // if the given element is very thin or flat the event handling element will be slightly bigger
	...
	this.getTooltip = function(element, tooltipText, colour, opacity){
		if (tooltipText != "") {
			...
			element.mouseover(function(event){
				tooltip.attr("x", event.clientX + 10 - elementPosition.left);
				text.attr("x", event.clientX + 15 - elementPosition.left);
				tooltipSet.toFront();
				tooltipSet.show();
			});
