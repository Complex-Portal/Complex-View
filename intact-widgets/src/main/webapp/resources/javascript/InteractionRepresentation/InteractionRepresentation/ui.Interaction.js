(function($){
    var interactionRepresentation = {
		...
		_DASStylesheet: 'http://www.ebi.ac.uk/das-srv/uniprot/das/uniprot/stylesheet',
		...
		// highlight a region on a protein by drawing a translucent rectangle
		highlightRegion: function(interactorId, x, x2, colour, text){
			this._drawer.highlightRegion(interactorId, x, x2, colour, text);
		},
        ...
        // highlight a feature by coloring the border black
		highlightFeature: function(featureId){
			this._drawer.highlightFeature(featureId);
		},

        // color border of currently highlighted feature in original colour
		unhighlightFeature: function(){
			this._drawer.unhighlightFeature();
		},

        // load all needed files and set number of requests
        _load: function(){
        ...
        // load colours for categories from stylesheet
