...
public class AdminBean implements Serializable {
    ...
    public void createIndexFromPsimitabFile(ActionEvent evt) {
        FacesContext facesContext = FacesContext.getCurrentInstance();
        try {
            facesContext.getExternalContext().getApplicationMap().put("fileupload_bytes", uploadedPsimitabFile.getBytes());
            facesContext.getExternalContext().getApplicationMap().put("fileupload_type", uploadedPsimitabFile.getContentType());
            facesContext.getExternalContext().getApplicationMap().put("fileupload_name", uploadedPsimitabFile.getName());
        } catch (IOException e) {
            e.printStackTrace();
        }

        if (log.isInfoEnabled()) log.info("Uploading file: " + uploadedPsimitabFile.getName());

        newIndex.setLocation(new File("/tmp", currentUser.getName() + "_" + uploadedPsimitabFile.getName()).toString());
