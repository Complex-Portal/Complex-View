...
public class GoHighlightmentSource extends HighlightmentSource {
    ...
    public Collection getKeysFromIntAct(String aProteinAC, HttpSession aSession) {
        ...
        try {
            if ( logger.isDebugEnabled() ) logger.debug( "Try to get a list of GO term (from protein AC="+ aProteinAC + ")" );
    ...
    private Collection filterInteractorXref(Collection xRef) {
        ...
        while ( xRefIterator.hasNext() ) {
            ...
            if ( ( xref.getCvDatabase().getShortLabel() ).toLowerCase().equals( SOURCE_KEY ) ) {
                ...
                if ( logger.isDebugEnabled() ) logger.debug( xref.getPrimaryId() );
            }
        }
    ...
    public List getSourceUrls( Collection xRefs, Collection selectedXRefs, String applicationPath, IntactUserI user )
        ...
        if ( logger.isDebugEnabled() ) logger.debug( listGOTerm.size() + " GO term(s) after filtering" );
...
//                                                                       "where x.parent.ac in (:acs) and x.primaryId = :id" );
        ...
        if ( listGOTerm != null && ( false == listGOTerm.isEmpty() ) ) {
            ...
            while ( list.hasNext() ) {
                ...
                if ( logger.isDebugEnabled() ) logger.debug( "Count of GO("+ goTermDescription +") is " + goTermCount );
