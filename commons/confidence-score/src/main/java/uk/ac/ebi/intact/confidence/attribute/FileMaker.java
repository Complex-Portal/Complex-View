...
public class FileMaker {
	...
	public void writeAnnotationAttributes(String notePath, String outPath) throws IOException {
		// write attributes based on UniProt annotation -- eg. GO and InterPro
		// terms
		...
		for (ProteinPair bi : biSet.getSet()) {
			...
			sb.append(names[0] + ";" + names[1]); // append protein names,
			...
			for (String term1 : annotation1) {
				for (String term2 : annotation2) {
					if (term1.equals(term2)) {
						...
						sb.append("," + term1 + ";" + term2); // comma-separated
																// list of term
																// pairs
					}
	...
	public void writeAlignmentAttributes(String hitPath, String hiconfPath, String outPath) throws IOException {
				...
				for (String p0 : hits0) {
					for (String p1 : hits1) {
						...
						if (hiConfSet.contains(hitPair)) {
							// this protein pair is related to a high-confidence
							// pair
	...
	private HashMap<String, String[]> getProteinAnnotations(String notePath) throws IOException {
		...
		while ((line = br.readLine()) != null) {
			...
			for (int i = 1; i < items.length; i++) { // subsequent items are
														// annotation terms
				terms[i - 1] = items[i];
			}
	...
	/**
	 * @param biSet the biSet to set
	 */
	public void setBiSet(BinaryInteractionSet biSet) {
		this.biSet = biSet;
	}
	 ...
	 * @return the biSet
	 */
	public BinaryInteractionSet getBiSet() {
		return biSet;
	}
