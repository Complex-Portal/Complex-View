...
public class InteractionGeneratorTest extends IntactBasicTestCase{
	...
	public void setUp() throws Exception {
		...
		intGen.setProteinACs(new DataMethods().exactRead(filepath));
		Collection<InteractionSimplified> highconf = initInteract(); 
		Collection<InteractionSimplified> lowconf = initInteract(); 
		Collection<InteractionSimplified> medconf = initInteract(); 
		intGen.setHighconfidence(highconf);
		intGen.setLowconfidence(lowconf);
		intGen.setMediumconfidence(medconf);
	}

	private Collection<InteractionSimplified> initInteract(){
	...
	private Collection<Interaction> initInteract2(){
	...
	public void testGenerate() throws Exception {
		Collection<InteractionSimplified> generated = intGen.generate(2);
		Assert.assertEquals(2, generated.size());
		printOut(generated);
	}

	private void printOut(Collection<InteractionSimplified> generated) {
		for (InteractionSimplified interaction : generated) {
			String out = interaction.getAc() + " : ";
			Collection<ProteinSimplified> comps = interaction.getInteractors();
